...
//             manager = Substitute.ForPartsOf<CameraManager>();
...
//             var result = manager.GetCameraZDistance();
...
//             var result = manager.DoCameraCentering(Vector3.zero);
...
//             var result = manager.DoAxisCameraMovement(horizontal, vertical, DeltaTime);
...
//             var result = manager.ClampCameraPos();
...
//         {
//             Vector3 mousePos = mainCamera.ViewportToScreenPoint(new Vector2(.5f, .5f));
//             var result = manager.DoCameraInputMovement(horizontal, vertical, mousePos, DeltaTime);
//             Vector3 expected = new Vector3(Mathf.Round(expectedX * 10000), 0f, Mathf.Round(expectedZ * 10000));
//             Vector3 roundedResult = new Vector3(Mathf.Round(result.x * 10000), Mathf.Round(result.y * 10000), Mathf.Round(result.z * 10000));
//             Assert.AreEqual(expected, roundedResult);
//         }
...
//         {
//             var result = manager.DoCameraPanning(new Vector2(mouseXAxis, mouseYAxis), DeltaTime);
...
//             var result = manager.DoCameraZooming(deltaScroll, DeltaTime);
