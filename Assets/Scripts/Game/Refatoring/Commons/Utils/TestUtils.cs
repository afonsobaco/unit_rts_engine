...
namespace Tests.Utils
{
    public class TestUtils
    {
        ...
        public static ISelectable[] GetSomeObjects(int amount)
        {
            return GetSomeObjects(amount, 0, default, default);
        }

        public static ISelectable[] GetSomeObjects(int amount, int startId)
        {
            return GetSomeObjects(amount, startId, default, default);
        }

        public static ISelectable[] GetSomeObjects(int amount, Type[] types)
        {
            return GetSomeObjects(amount, 0, types, default);
        }
        public static ISelectable[] GetSomeObjects(int amount, int startId, Type[] types)
        {
            return GetSomeObjects(amount, startId, types, default);
        }

        public static ISelectable[] GetSomeObjects(int amount, int startId, Type[] types, object[] constructorParams)
        {
            if (types == null)
            {
                types = new Type[] { };
            }
            if (!types.Contains(typeof(ISelectable)))
            {
                var t = new List<Type>(types);
                t.Add(typeof(ISelectable));
                types = t.ToArray();
            }
            var list = new List<ISelectable>();
            for (var i = 0; i < amount; i++)
            {
                ISelectable item = (ISelectable)Substitute.For(types, constructorParams);
                item.Index = i + startId;
                list.Add(item);
            }
            return list.ToArray();
        }
